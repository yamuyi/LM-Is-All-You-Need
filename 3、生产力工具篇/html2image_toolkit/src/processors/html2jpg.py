...
# from src.config import DEFAULT_WATERMARK, SEGMENTED_OUTPUT_DIR
...
#             logger.info(f"页面尺寸: {total_width} x {total_height}")
...
#         if style == "grid":
#             return self._add_grid_watermark(image, watermark_text, **kwargs)
#         else:
#             return self._add_density_controlled_watermark(image, watermark_text, **kwargs)
    
#     def _add_density_controlled_watermark(self, image: Image.Image, watermark_text: str, **kwargs) -> Image.Image:
#         """密度可控水印"""
#         # 合并默认参数
#         params = {
#             'font_size': DEFAULT_WATERMARK['font_size'],
...
#             # 获取字体
#             font = self._get_chinese_font(params['font_size'])
...
#             fill_color = (*params['color'], int(255 * params['opacity']))
...
#             # 计算间距
#             spacing_x = int(rot_width * params['spacing_ratio'])
#             spacing_y = int(rot_height * params['spacing_ratio'])
...
#             # 计算网格间距
#             cell_width = width // params['grid_columns']
#             cell_height = height // params['grid_rows']
            
#             # 水印样式
#             fill_color = (255, 255, 255, int(255 * params['opacity']))
            
#             # 网格布局放置水印
#             for col in range(params['grid_columns'] + 1):
#                 for row in range(params['grid_rows'] + 1):
#                     x = col * cell_width
#                     y = row * cell_height
...
#     def process_html(self, html_file: str | Path, **kwargs) -> dict | None:
...
#         output_dir = SEGMENTED_OUTPUT_DIR / f"{file_name}_{timestamp}"
        
#         try:
#             # 1. HTML转长图片
#             long_image_path = output_dir / "long_image.png"
#             long_image = self.html_to_long_image(html_file, long_image_path)
#             if not long_image:
#                 return None
            
#             # 2. 加水印
#             watermarked_path = output_dir / "watermarked_image.png"
