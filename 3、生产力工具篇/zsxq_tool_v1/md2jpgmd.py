...
def download_image(url, download_dir=IMAGES_DIR):
    """下载图片并保存到指定目录"""
    try:
        # 解析URL获取文件名
...
def create_zip_file(markdown_content, original_filename, url_map, original_content=None):
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            # 添加图片文件夹中的所有图片
            for image_file in task_images_dir.rglob("*"):  # 使用rglob递归查找
            ...
            if st.button("🚀 开始处理", type="primary", use_container_width=True):
                with st.spinner("正在处理中，请稍候..."):
                    ...
                    for i, url in enumerate(urls_to_download):
                        ...
                        filename, filepath = download_image(url)
                    ...
                    # 替换Markdown内容中的图片链接 - 使用原始内容进行替换
                    modified_content = replace_image_urls(st.session_state.original_content, url_map)
                    
                    # 对比原始内容和修改后内容
                    original_lines = st.session_state.original_content.split('\n')
                    modified_lines = modified_content.split('\n')
                    
                    if len(original_lines) != len(modified_lines):
                        st.warning(f"⚠️ 注意：行数变化 (原始: {len(original_lines)}行, 修改后: {len(modified_lines)}行)")
                    ...
                    try:
                        zip_path, task_id, zip_image_count = create_zip_file(
                            ...
                            uploaded_file.name, 
                            url_map,
                            st.session_state.original_content  # 传递原始内容用于保持格式
                        )
                    except Exception as e:
                    ...
                    with col2:
                        ...
                        # 显示格式信息
                        st.info(f"原始文件行数: {len(original_lines)}")
                        st.info(f"修改后行数: {len(modified_lines)}")
...
└── images/
    ├── {', '.join([list(url_map.values())[i].split('/')[1] for i in range(min(3, len(url_map)))])}
    {'...' if len(url_map) > 3 else ''}
                            """
                            st.code(zip_structure.strip())
                        
                        # 清理提示
                        st.info(f"💡 ZIP文件将在24小时后自动清理。任务ID: {task_id}")
                            ...
                            with test_col1:
                                if st.button("查看测试内容"):
                                    ...
                                    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                                        # 读取修改后的markdown文件
                                        md_content = zip_ref.read(f"{uploaded_file.name.split('.')[0]}_local.md").decode('utf-8')
                            ...
                            with test_col2:
                                if st.button("检查images文件夹"):
                                    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                                        ...
                                        st.write(f"Images文件夹中的文件: {len(image_files)}")
